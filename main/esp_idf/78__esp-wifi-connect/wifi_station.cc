...
void WifiStation::Stop() {
    ...
    // 先设置sta_active_为false，防止事件处理中尝试重连
    sta_active_ = false;
...
void WifiStation::Start() {
    ...
    if(custom_mac[0] == 0 && custom_mac[1] == 0 &&
       custom_mac[2] == 0 && custom_mac[3] == 0 &&
       custom_mac[4] == 0 && custom_mac[5] == 0) {
        // Use the default MAC address
        ESP_LOGI(TAG, "Using default MAC address");
    } else {
        esp_wifi_set_mac(WIFI_IF_STA, custom_mac);
        ESP_LOGI(TAG, "Using custom MAC address: %02x:%02x:%02x:%02x:%02x:%02x",
                 custom_mac[0], custom_mac[1], custom_mac[2],
                 custom_mac[3], custom_mac[4], custom_mac[5]);
    }
    // 修正：sta_active_ 必须在 esp_wifi_start() 之前赋值为 true
    sta_active_ = true;
...
//     // 修正：仅在 Stop/Deinit 后立即 return，Start 后事件必须能正常处理
//     // 不要在 Start 后的事件阶段阻断事件处理
//     if (!this_->sta_active_ && event_id != WIFI_EVENT_STA_START) {
//         ESP_LOGW(TAG, "STA not active, ignore event %ld", event_id);
//         return;
//     }
//     // 不要在这里用 wifi_station_started 判断，否则事件会被忽略
...
//         // 修正：断开时如果sta_active_为false，直接return，绝不再调用esp_wifi_connect
//         if (!this_->sta_active_) {
//             ESP_LOGW(TAG, "STA not active, skip reconnect");
//             return;
//         }
...
void WifiStation::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    ...
    if (esp_wifi_get_mode(&mode) != ESP_OK || mode == WIFI_MODE_NULL) {
        ESP_LOGW(TAG, "WiFi not initialized, ignore event %ld", event_id);
        return;
    }
    ...
    if (event_id == WIFI_EVENT_STA_START) {
        ...
        // 增加WiFi状态双重检查
        if (!this_->sta_active_ || esp_wifi_get_mode(&mode) != ESP_OK || mode == WIFI_MODE_NULL) {
            ESP_LOGW(TAG, "STA not active or WiFi stopped, skip reconnect");
            return;
        }
        ...
        if (this_->reconnect_count_ < MAX_RECONNECT_COUNT) {
            ...
            if (err != ESP_OK) {
                ESP_LOGE(TAG, "Failed to reconnect, error: %d", err);
                return;
            }
            this_->reconnect_count_++;
            ESP_LOGI(TAG, "Reconnecting %s (attempt %d / %d)", this_->ssid_.c_str(), this_->reconnect_count_, MAX_RECONNECT_COUNT);
